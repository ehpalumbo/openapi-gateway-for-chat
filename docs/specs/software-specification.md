...
| ID | Requirement |
| ---- | ------------- |
| R-RESP-1 | Every response carrying an HTTP status is served as a uniform two-part tool result: a metadata **text part** containing the raw HTTP head as plain text — bare status line (`<status> [<statusText>]`), headers in arrival order as `key: value` lines (lower-case as returned by `fetch()`), then a blank line (`statusLine\nheaders\n\n`) — followed by the body routed by content type per R-RESP-3. |
...
4. **Response serving mechanics (2026-08-26, rev. 3):** every HTTP response is a two-part tool result — text-part metadata as plain text mimicking the raw HTTP head (bare status line, headers in arrival order as `key: value` lower-case, blank line — `statusLine\nheaders\n\n`) first, then the body routed by content type: UTF-8 text part for textual types, image `LanguageModelDataPart` for vision-safe images, spill file under `<storageUri>/response-spills/` for non-image binaries (deleted on `deactivate`). Constraint driving the shape: Copilot Chat forwards only text parts and image data parts from tool results into the model prompt (microsoft/vscode#275300). See the design history in §3.8 for the two superseded iterations. Rev. 4 (2026-08-26): metadata switched from JSON `{status, statusLine, headers}` to plain-text raw head — no numeric `status` field, bare status line. No backwards compatibility (v0).
