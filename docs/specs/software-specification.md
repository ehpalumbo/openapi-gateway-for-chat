...
| ID | Requirement |
| ---- | ------------- |
| R-RESP-1 | Every response carrying an HTTP status is served as a metadata **text part** containing the raw HTTP head as plain text — bare status line (`<status> [<statusText>]`), headers in arrival order as `key: value` lines (lower-case as returned by `fetch()`), then a blank line (`statusLine\nheaders\n\n`) — **always present** as the first tool-result part, followed — **only when the response has a body** — by a second part with the body routed by content type per R-RESP-3. Responses without a body (strict `byteLength===0`, e.g. `204 No Content`, empty `404`) return only the metadata part. |
| R-RESP-2 | Non-2xx statuses are **not** special-cased: they follow the same shape, with the status readable from the metadata part and the error body — when present — served whole with no truncation or capping; empty error bodies return only the metadata part. |
| R-RESP-3 | When a body is present, bodies are routed by content type: textual types (`text/*`, `application/json`, `+json`) become a text part with the UTF-8 body; vision-safe images (`image/png`, `image/jpeg`, `image/gif`, `image/webp`, `image/bmp`) become an image `LanguageModelDataPart`; every other binary body is written via **`vscode.workspace.fs`** to `<storageUri>/response-spills/<apiId>-<operationId>-<uuid>.<ext>` (unique names so concurrent/repeated calls never override each other) and referenced from a text part with content type, byte size, and absolute path. Spilled files are removed on extension `deactivate`. |
...
4. **Response serving mechanics (2026-08-26, rev. 3):** every HTTP response is a metadata text part as plain text mimicking the raw HTTP head (bare status line, headers in arrival order as `key: value` lower-case, blank line — `statusLine\nheaders\n\n`) — always present — followed, only when the response has a body, by a second part with the body routed by content type: UTF-8 text part for textual types, image `LanguageModelDataPart` for vision-safe images, spill file under `<storageUri>/response-spills/` for non-image binaries (deleted on `deactivate`). Responses without a body (strict `byteLength===0`, e.g. `204`, empty `404`) return only the metadata part. Constraint driving the shape: Copilot Chat forwards only text parts and image data parts from tool results into the model prompt (microsoft/vscode#275300). See the design history in §3.8 for the two superseded iterations. Rev. 4 (2026-08-26): metadata switched from JSON `{status, statusLine, headers}` to plain-text raw head — no numeric `status` field, bare status line. Rev. 5 (2026-08-27): metadata remains always present as first part; body part present only when there is a body (fixes zero-byte binary spill for body-less responses). No backwards compatibility (v0).
